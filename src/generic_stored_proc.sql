USE SCHEMA MTD;
CREATE or replace PROCEDURE proc_create_tables(CATEGORY STRING)
 returns float
 language javascript
 as
   $$
...
||', KAFKA_DTS TIMESTAMP_LTZ, LOAD_DTS TIMESTAMP_LTZ,  RECORD_SRC varchar'
||')'
) AS DDL_CMD
...
where l.version_id = (select max(version_id) FROM MTD.MTD_MODEL_CREATION_LOG where approval = 1)
  ...
  GROUP BY link_table_name,l.NON_HIST_LINK,hub_table_hk;`});
...
GROUP BY s.sat_table_name,sub_table.HK,s.HASH_DIFF;`});
