USE SCHEMA MTD;
CREATE or replace PROCEDURE proc_etl_hub(INIT_FG VARCHAR)
 returns string
 language javascript
 as
   $$
...
||', min(to_timestamp_ltz(NVL(TRIM(RECORD_METADATA:LogAppendTime,''\\\'"\\\'''),TRIM(0)))), to_timestamp_ltz(current_timestamp) as LOAD_DTS,'
...
||IFF(src_c.array_path is not null,',lateral flatten(input => '||IFF(SRC_VALUE_COLUMN is not null,TRIM(SRC_VALUE_COLUMN)||':','')||src_c.array_path||')','') || ' WHERE ' ||IFF(`+INIT_FG+`,' 1=1 ','METADATA$ACTION = ''\\\'INSERT\\\''' ') || IFF(src_c.FILTER_CRITERIA <> 'PII',src_c.FILTER_CRITERIA,'') ||' )src 
WHERE hash_key_calc NOT IN
(SELECT ' 
||substr(hub_table_name,1,length(hub_table_name)-position('_' IN reverse(hub_table_name)))||'_HK'
||' FROM EDW.'|| hub_table_name || ') GROUP BY hash_key_calc, LOAD_DTS,  RECORD_SRC'
...
GROUP BY hub_table_name,src_t.src_table_database,src_t.src_table_schema,src_t.src_table_name,h_m.src_table_id, h.version_id, hk.ddl,es.etl_sql,src_t.src_value_column,src_c.array_path,src_c.FILTER_CRITERIA
