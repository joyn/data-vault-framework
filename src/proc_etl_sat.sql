USE SCHEMA mtd;
CREATE or replace PROCEDURE proc_etl_sat(INIT_FG VARCHAR)
 returns string
 language javascript
 as
   $$
...
||'to_timestamp_ltz(NVL(TRIM(RECORD_METADATA:LogAppendTime,''\\\'"\\\'''),TRIM(0))), to_timestamp_ltz(current_timestamp) ,'
...
||IFF(hk_s.array_path is not null and hk_s.array_path != '',',lateral flatten(input => '||IFF(SRC_VALUE_COLUMN is not null,TRIM(SRC_VALUE_COLUMN)||':','')||hk_s.array_path||')','') || ' WHERE ' ||IFF(`+INIT_FG+`,' 1=1 ','METADATA$ACTION = ''\\\'INSERT\\\''' ') || IFF(src_c.FILTER_CRITERIA <> 'PII',src_c.FILTER_CRITERIA,'') ||' )src '
||IFF(s.HASH_DIFF,' LEFT JOIN ( SELECT '|| sub_table.HK ||', hash_diff FROM EDW.'|| sat_table_name ||' as tgt 
      JOIN (SELECT '|| sub_table.HK ||' as max_hk ,max(KAFKA_DTS) as MAX_KAFKA_DTS FROM EDW.'|| sat_table_name ||' GROUP BY 1) as tgt_max
      ON tgt.'|| sub_table.HK ||' = tgt_max.max_hk AND tgt.KAFKA_DTS = tgt_max.MAX_KAFKA_DTS ) tgt_out
      ON '|| hk_s.ddl ||' = '|| sub_table.HK ||' WHERE (hash_diff_calc <> hash_diff OR hash_diff is null) AND ' || hk_s.null_logic ,' WHERE '|| hk_s.null_logic)
...
GROUP BY sat_table_name,src_t.src_table_database,src_t.src_table_schema,src_t.src_table_name,s.src_table_id, s.version_id, SUB_TABLE.HK,s.HASH_DIFF,HK_S.DDL,es.etl_sql,hk_s.array_path,SRC_T.SRC_VALUE_COLUMN,hk_s.null_logic,src_c.FILTER_CRITERIA
