CREATE or replace PROCEDURE mtd.proc_etl_link(INIT_FG VARCHAR)
 returns string
 language javascript
 as
   $$
...
||hub_hk
||IFF(l.NON_HIST_LINK,
      nl_src_col||IFF(kafka_dts, ',to_timestamp_ltz(NVL(TRIM(RECORD_METADATA:LogAppendTime,''\\\'"\\\'''),TRIM(0)))', ''),
...
||IFF(hk_l.array_path is not null and hk_l.array_path != '',',lateral flatten(input => '||IFF(SRC_VALUE_COLUMN is not null,TRIM(SRC_VALUE_COLUMN)||':','')||hk_l.array_path||')','')|| ' WHERE 1=1 ' || IFF(hk_l.FILTER_CRITERIA <> 'PII',hk_l.FILTER_CRITERIA,'') ||' )src '
||IFF(l.NON_HIST_LINK,'',' 
 WHERE hash_key_calc NOT IN ( SELECT ' ||substr(link_table_name,1,length(link_table_name)-position('_' IN reverse(link_table_name)))||'_HK FROM EDW.'|| link_table_name ||' ) 
 GROUP BY hash_key_calc,' || hub_hk)
 AS out_etl_sql
