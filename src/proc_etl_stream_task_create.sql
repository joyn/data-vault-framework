USE SCHEMA UTIL;
CREATE or replace PROCEDURE proc_etl_stream_task_create()
returns string
language javascript
as
 $$
...
SELECT 
upper('CREATE STREAM IF NOT EXISTS STREAM_' || es.SRC_TABLE_NAME|| '_TO_' || es.TGT_TABLE_NAME || ' ON TABLE ' || src_t.SRC_TABLE_DATABASE ||'.'||  src_t.SRC_TABLE_SCHEMA || '.' || es.SRC_TABLE_NAME||';') AS STREAM_DDL,
